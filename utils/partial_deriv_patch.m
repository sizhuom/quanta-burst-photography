function [It, Ix, Iy] = partial_deriv_patch(img1, img2, uv_prev, interpolation_method, deriv_filter, b)

%PARTIAL_DERIV_PATCH   Spatio-temporal derivatives for aligning a template
%image img1 to another image img2, which may not have the same sizes
%   P = PARTIAL_DERIV_PATCH(IMAGES, INIT) computes the spatio-temporal derivatives
%
...
if strcmp(interpolation_method, 'bi-cubic')
    ...
    if nargout == 1
        ...
        if size(img1, 3) == 1
            % gray-level
            warpIm = interp2_bicubic(img2,x2,y2, h);
        else
            % color
            warpIm  = zeros(size(img1));
            for j = 1:size(img1,3)
                warpIm(:,:,j) = interp2_bicubic(img2(:,:,j),x2,y2, h);
            end
        end
    ...
    elseif nargout == 3
        ...
        if size(img1, 3) == 1
            % gray-level
            [warpIm, Ix, Iy] = interp2_bicubic(img2,x2,y2, h);
        else
            % color
            warpIm  = zeros(size(img1));
            Ix      = warpIm;
            Iy      = warpIm;
            for j = 1:size(img1,3)
                [warpIm(:,:,j), Ix(:,:,j), Iy(:,:,j)] = interp2_bicubic(img2(:,:,j),x2,y2, h);
            end
        end
...
elseif strcmp(interpolation_method, 'bi-linear') || strcmp(interpolation_method, 'cubic') 
    ...
    if size(img1, 3) == 1
        % Gray-level
        warpIm  = interp2(img2,x2,y2,method);
        It      = warpIm - img1;      
        ...
        warpIm  = zeros(size(img1));
        for j = 1:size(img1,3)
            warpIm(:,:,j) = interp2(img2(:,:,j),x2,y2,method, NaN);
        end
        It      = warpIm - img1;
...
%             Iy  = interp2_bicubic(I2y,x2,y2, h);
            ...
            for j = 1:size(images,3)
                Ix(:,:,j)  = interp2(I2x(:,:,j),x2,y2,method);
                Iy(:,:,j)  = interp2(I2y(:,:,j),x2,y2,method);
            end
...
%         Iy        = imfilter(warpIm, h', 'corr', 'symmetric', 'same');
    ...
    % Disable those out-of-boundary pixels in warping
%     It(B)   = 0;
    mask = ~isfinite(It) | ~isfinite(Ix) | ~isfinite(Iy);
    It(mask) = 0;
    if nargout == 3
%         Ix(B) = 0;
%         Iy(B) = 0;
