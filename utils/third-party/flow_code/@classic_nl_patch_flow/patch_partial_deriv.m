function [It, Ix, Iy] = patch_partial_deriv(this, uv_prev, b)
...
images = this.images;
interpolation_method = this.interpolation_method;
h = this.deriv_filter;
% pad = max(ceil((size(h)-1)/2));
...
hs = size(uv_prev, 1);
ws = size(uv_prev, 2);
patchStride = this.patchStride;
patchSize = this.patchSize;
% xfv = repelem((0:ws-1)*patchStride,1,patchSize+pad*2)+repmat(1-pad:patchSize+pad,1,ws);
% yfv = repelem((0:hs-1)*patchStride,1,patchSize+pad*2)+repmat(1-pad:patchSize+pad,1,hs);
% xbv = repelem((0:ws-1)*(patchSize+pad*2),1,patchSize)+repmat(1+pad:patchSize+pad,1,ws);
% ybv = repelem((0:hs-1)*(patchSize+pad*2),1,patchSize)+repmat(1+pad:patchSize+pad,1,hs);
xfv = repelem((0:ws-1)*patchStride,1,patchSize)+repmat(1:patchSize,1,ws);
yfv = repelem((0:hs-1)*patchStride,1,patchSize)+repmat(1:patchSize,1,hs);

[x,y]   = meshgrid(xfv, yfv);
% x2      = x + repelem(uv_prev(:,:,1), patchSize+pad*2, patchSize+pad*2);        
% y2      = y + repelem(uv_prev(:,:,2), patchSize+pad*2, patchSize+pad*2);  
x2      = x + repelem(uv_prev(:,:,1), patchSize, patchSize);        
y2      = y + repelem(uv_prev(:,:,2), patchSize, patchSize);  
...
if strcmp(interpolation_method, 'bi-linear') || strcmp(interpolation_method, 'cubic') 
    ...
    if size(images, 4) == 1
        % Gray-level
        warpIm2  = interp2(images(:,:,2),x2,y2,method);
        It      = warpIm2 - img1(yfv, xfv);      
        
    else
        % Color
        error('Not implemented!');
%         warpIm2  = zeros(size(images(:,:,:,1)));
%         for j = 1:size(images,3)
%             warpIm2(:,:,j) = interp2(x,y,images(:,:,j,2),x2,y2,method, NaN);
%         end;
%         It      = warpIm2 - images(:,:,:,1);
...
%         end;
    ...
    if nargout == 3
        ...
        if size(images, 4) == 1
            % Gray-level
            Ix  = interp2(I2x,x2,y2,method);
            Iy  = interp2(I2y,x2,y2,method);
...
%         Iy        = imfilter(warpIm, h', 'corr', 'symmetric', 'same');
        ...
        Ix  = b*Ix+(1-b)*I1x(yfv, xfv);
        Iy  = b*Iy+(1-b)*I1y(yfv, xfv);        
