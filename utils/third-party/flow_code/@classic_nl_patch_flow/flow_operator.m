...
% THIS SOFTWARE.        
  ...
  It = It + Ix.*repmat(duv(:,:,1), [this.patchSize this.patchSize size(It,3)]) ...
          + Iy.*repmat(duv(:,:,2), [this.patchSize this.patchSize size(It,3)]);
  ...
  tmp = aggregatePatch(pp_d.*Ix2(:), this.patchSize, size(It));
  duu = spdiags(tmp, 0, npixels, npixels);
  tmp = aggregatePatch(pp_d.*Iy2(:), this.patchSize, size(It));
  dvv = spdiags(tmp, 0, npixels, npixels);
  tmp = aggregatePatch(pp_d.*Ixy(:), this.patchSize, size(It));
  dduv = spdiags(tmp, 0, npixels, npixels);

  A = [duu dduv; dduv dvv] - this.lambda*M;

  % right hand side
  b =  this.lambda * M * uv(:) - [aggregatePatch(pp_d.*Itx(:), this.patchSize, size(It)); aggregatePatch(pp_d.*Ity(:), this.patchSize, size(It))];
...
function A = aggregatePatch(I, patchSize, imsz)
  scaleFilter = ones(patchSize) / patchSize^2;
  scaleCenter = floor((patchSize+1)/2);
  A = imfilter(imresize(I, imsz), scaleFilter);
  A = A(scaleCenter:patchSize:end,scaleCenter:patchSize:end);  
  A = A(:);
end