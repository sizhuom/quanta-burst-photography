...
% THIS SOFTWARE.        
  ...
  % Frame size
  h = size(this.images, 1);
  w = size(this.images, 2);
  
  % Flow field size
  numStrides = this.patchSize / this.patchStride;
  assert(~mod(numStrides, 1));
  hs = (h-this.patchSize)/this.patchStride+1;
  ws = (w-this.patchSize)/this.patchStride+1;
