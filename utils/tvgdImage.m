function reI = tvgdImage(S, T, scale, params)
...
if ~isinf(params.maxCounts)
    S(S > params.maxCounts) = params.maxCounts;
    xmax = mleImage(params.maxCounts, T);
else
    xmax = Inf;
end

x = mleImage(S, T); % initialize as MLE
% x = zeros(size(S));
% x(~isfinite(x)) = 0;
x(x<=0) = 0.2;
x = x(:);
...
if order == 0
    ...
    f = (-params.s/params.T./(1-exp(-x))+1)'...
        ...
        +params.lambda*sign(params.Dy*x)'*params.Dy;
end
end
